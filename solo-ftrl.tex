...
The closest algorithm to a scale-free one in the OLO literature is the \textsc{AdaGrad}
...
other words, despite of its name, \textsc{AdaGrad} is not fully adaptive to the norm of
the loss vectors. Identical considerations hold for the \textsc{FTRL-Proximal} in
...
feeling that \textsc{FTRL} needs to be ``fixed'' in order to obtain a scale-free
algorithm.
...
Corollary~\ref{corollary:ada-ftrl-regret-bound} for \textsc{AdaFTRL}. The scaling
...
convex with respect to $\|\cdot\|$. \textsc{SOLO FTRL} with regularizer
$$
...
\end{lemma}
%
Lemma~\ref{lemma:sum-of-square-roots-inverses} is from \cite[Lemma
3.5]{Auer-Cesa-Bianchi-Gentile-2002}.  We give
a proof in~\ref{section:solo-ftrl-proofs} for completeness.

\begin{proof}[Proof of Theorem~\ref{theorem:regret-solo-ftrl}]
Let $\eta_t=\tfrac{1}{\sqrt{\sum_{i=1}^{t-1} \|\ell_i\|_*^2}}$, hence $R_t(w) = \tfrac{1}{\eta_t} R(w)$.
